...
function keep_web_alive() {
  ...
  exec("pgrep -laf app.js", function (err, stdout, stderr) {
    // 1.查后台系统进程，保持唤醒
    if (stdout.includes("./app.js server --no-prefix")) {
      console.log("web 正在运行");
    }
    else {
      //web 未运行，命令行调起
      exec(
        "chmod +x app.js && ./app.js server --no-prefix >/dev/null 2>&1 &", function (err, stdout, stderr) {
          if (err) {
